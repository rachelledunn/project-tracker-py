...
def get_project_by_title(title):
    ...
    row = db_cursor.fetchone()
    print "The %s project description is %s" % (row[0], row[1])
...
def get_grade(github, title):
    ...
    row = db_cursor.fetchone()
    print "The student's grade on %s was %s" % (title, row[0])

def give_grade(first_name, title, grade):
    """ Give a grade to a student. """

    QUERY = """
        
        INSERT INTO Grades VALUES ((SELECT github 
            FROM Students WHERE first_name = ?), ?, ?)
        """

    db_cursor.execute(QUERY, (first_name, title, grade))
    db_connection.commit()
...
def handle_input():
    ...
    while command != "quit":
        ...
        if command == "student":
            ...
            first_name, title, grade = args
            give_grade(first_name, title, grade)
